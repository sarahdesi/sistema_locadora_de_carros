...
new class extends Component {
    ...
    // Propriedades dinâmicas calculadas
    public $dias_locacao = 0;
    public $valor_base_calculado = 0;
    ...
    {
        // Força o recálculo imediato na tela sempre que alterarem datas, km ou custos extras
    ...
    {
        // Segurança: Caso não encontre o registro de check-in, interrompe para não quebrar
        if (!$this->contrato->checkIn) {
            return;
        }

        $saida = Carbon::parse($this->contrato->checkIn->data_hora_saida);
        $previsto = Carbon::parse($this->contrato->data_hora_retorno);
        $real = Carbon::parse($this->data_real_entrega);
        $valorDiaria = (float) $this->contrato->valor_diaria;

        // --- REGRA DE NEGÓCIO: CÁLCULO DOS DIAS E VALOR BASE (OPÇÃO A) ---
        // Se a entrega real for DEPOIS da previsão, calcula os dias normais até a previsão
        if ($real->gt($previsto)) {
            $this->dias_locacao = max(1, ceil($saida->diffInHours($previsto) / 24));
            $this->valor_base_calculado = $this->dias_locacao * $valorDiaria;

            // Calcula o excedente como multa por atraso
            $this->dias_atraso = max(1, ceil($previsto->diffInHours($real) / 24));
            $this->valor_multa_atraso = $this->dias_atraso * ($valorDiaria * 1.2); // Diária + 20%
        } else {
            // Se entregou antes ou no prazo, cobra apenas as diárias do período utilizado real
            $this->dias_locacao = max(1, ceil($saida->diffInHours($real) / 24));
            $this->valor_base_calculado = $this->dias_locacao * $valorDiaria;
        ...
        // Soma final: Base real + Multas de Atraso + Custos Extras informados na hora
        $this->valor_final = $this->valor_base_calculado + $this->valor_multa_atraso + (float) ($this->custo_adicional ?? 0);
    }
    ...
    public function fecharContrato()
    {
        ...
        // Atualiza a tabela contratos com os dados estruturados do encerramento
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                    ...
                    <select wire:model="avaliacao_limpeza" required
                            class="w-full border border-gray-300 rounded-lg px-3 py-1.5 text-sm bg-white focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                        <option value="Excelente">Não Necessita de Lavagem</option>
                        <option value="Regular">Sujeira leve</option>
                        <option value="Ruim">Necessita Lavagem Pesada</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 p-3 rounded-xl border border-gray-100 space-y-3">
            ...
            <div class="border-t border-gray-200 pt-2 space-y-1 text-xs">
                <div class="flex justify-between text-gray-500">
                    <span>Valor Base Original ({{ $dias_locacao }} {{ $dias_locacao > 1 ? 'diárias' : 'diária' }}):</span>
                    <span class="font-mono text-gray-700 font-medium">R$ {{ number_format($valor_base_calculado, 2, ',', '.') }}</span>
                </div>
